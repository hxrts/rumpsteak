...
use crate::effects::algebra::{Program, Effect, InterpretResult, InterpreterState, ProgramMessage};
        ...
        M: ProgramMessage + Serialize + DeserializeOwned + 'static,
    {
        match effect {
            ...
            Effect::Recv { from, msg_type } => {
                ...
                match self.try_recv_as_type::<H, R, M>(handler, endpoint, from).await {
                    Ok(value) => {
                ...
                if !matches!(result.final_state, InterpreterState::Completed) {
                    match result.final_state {
                        ...
                        InterpreterState::Timeout => {
                            return Err(ChoreographyError::Timeout(std::time::Duration::from_secs(0)));
                        }
            ...
            Effect::Timeout { at, dur, body } => {
                ...
                match tokio::time::timeout(dur, Box::pin(self.run(handler, endpoint, *body))).await {
                    Ok(Ok(result)) => {
    ...
    impl<R: RoleId + 'static> ChoreoHandler for MockHandler<R> {
        ...
        async fn recv<M: DeserializeOwned + Send>(
            ...
            if let Some(MockResponse::Message(bytes)) = self.scripted_responses.pop_front() {
                ...
                Err(ChoreographyError::Transport("No scripted response available".into()))
            }
        }
        ...
        async fn offer(
            ...
            if let Some(MockResponse::Label(label)) = self.scripted_responses.pop_front() {
                ...
                Err(ChoreographyError::Transport("No scripted label available".into()))
            }
        }
...
mod tests {
    ...
    async fn test_simple_program() {
        ...
        let mut endpoint = ();
        let result = interpret(&mut handler, &mut endpoint, program).await.unwrap();
